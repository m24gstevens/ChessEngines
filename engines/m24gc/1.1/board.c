...
void parse_fen(char *fen) {
    ...
    hash = generate_hash();
    material_count = 0;
    for (int pc = K; pc <= q; pc++)
        material_count += popcount(bitboards[pc]) * simple_piece_values[pc];  
}
...
int make_move(U16 move) {
    ...
    game_history[hply].material = material_count;
    ...
    if (flags & 0x8) {
        ...
        // Material
        material_count -= simple_piece_values[P + 6*side_to_move];
        material_count += simple_piece_values[promoted_piece];
    }
    if (flags == 2) {
        /* Kingside castles */
