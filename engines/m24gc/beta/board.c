...
void parse_fen(char *fen) {
    ...
    for (int pc = P; pc <= Q; pc++) {
        material_count[0] += popcount(bitboards[pc]) * simple_piece_values[pc];
        material_count[1] += popcount(bitboards[pc + 6]) * simple_piece_values[pc]; 
    }
...
// Generate captures when we are in a single check
void generate_captures_in_check(U64 checkers) {
    U64 bitboard;
    int target;
    ...
    if (checkers & (checkers - 1)) {
        ...
        U64 attacks = king_attack_table[source] & (side_to_move ? occupancies[WHITE] : occupancies[BLACK]);
        while (attacks) {
        ...
        // This time, the checkmask is exactly the checkers (So we must make a capture)
        U64 checkmask = checkers;
...
void generate_captures() {
    ...
    if (checkers) {
        // If we are in a single check, generate the captures. Otherwise, we are in a double check
        generate_captures_in_check(checkers);
...
/* Legal moves */
void generate_moves_given_checkers(U64 checkers) {
    U64 occ_both = occupancies[BOTH];
    U64 occ_white = occupancies[WHITE];
    U64 occ_black = occupancies[BLACK];
    moves_start_idx[ply+1] = moves_start_idx[ply];
...
// Generate moves not given the checkers
void generate_moves() {
    U64 checkers = all_checkers();
    generate_moves_given_checkers(checkers);
}
...
int make_move(U16 move) {
    ...
    if (flags & 0x4) {
        ...
        if (flags == 5) {
            ...
            // Material update
            material_count[1 ^ side_to_move] -= simple_piece_values[P];
        } else {
            bitboards[taken_piece] ^= ((U64)1 << move_to);
            occupancies[1 ^ side_to_move] ^= ((U64)1 << move_to);
            occupancies[BOTH] ^= ((U64)1 << move_to);
            // Material update
            material_count[1 ^ side_to_move] -= positive_simple_piece_values[taken_piece];
        }
    }
    if (flags & 0x8) {
        /* Promotion */
        ...
        // Material
        material_count[side_to_move] -= simple_piece_values[P];
        material_count[side_to_move] += positive_simple_piece_values[promoted_piece];
    }
    if (flags == 2) {
        /* Kingside castles */
